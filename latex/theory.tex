...
% The genome gives the ordering of the elements, the benchmark format gives what values are available. Abstractly, the fitness should be the sum of servicing each element as one reaches it in the order the genome dictates, plus the distance/cost of moving between each of them. This can be expressed as the sum of 
% Finding the cost of going between elements that do not require servicing 



% Fintss Components -> calculations -> (FLW) assumptions about G -> FLW assumptions about passthrough - FLW pseudocode.



% When the components neccessary to compute the fitness have been obtained, there is one more thing that needs to be in place before the process of calculation is determined. How the connection between the road network, the fitness components, and the genome should work. The format used by the NEARP benchmarks provides a nice way to tie the road network and fitness components together into a graph with nodes, edges, and arcs that can be used as the EAs internal representation.

% The cost component 

% Due to the problem being approached as a NEARP, the natural way to do this is to use a graph with nodes, edges, and arcs as the EAs internal representation.

% The existing NEARP benchmarks provide 
% The format used by the NEARP benchmarks provides a good way of combining the constraints and the road network into such a graph.
% facilitates, provides, supplies, 



% Underlying graph -> graph as set -> required elements.
% How parameters should be converted to pass-through/dead-heading cost, servicing cost, and demand
% A short bit about the penalize demand out of order through lin-reg idea
