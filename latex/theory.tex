...
\citet{wohlk2008decade} presents that the CPP, if it contains only undirected edges or directed edges (arcs) (G=\{N,E\} or G=\{N,A\}), can be solved in polynomial time. However, if it is a mixed graph containing both (G=\{N,E,A\}) it is NP-hard. The related VRP also being NP-hard has interesting implication for route optimization problem in general. Because many problems can be reduced to either the VRP or CPP they are by extension NP-hard too. The NP-hardness makes heuristic approaches the most feasible way of obtaining good solutions. Coupled with the complexity of modelling real-life situations most attempts at pushing the state of the art focus on either improving the algorithm's or the models.
