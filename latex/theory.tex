...
% When calculating the fitness for snow plowing, there are two important aspects. Firstly what factors should be taken into consideration, and how should they be weighted against each other. Secondly, how should the calculations be done in the EAs representation of the problem once the parameters are known.

% a natural palce to start would be finding what needs to be represented before looking at how to do math with it

% For the calculations using the representation of the problem to make sense, we should first define what the problem is

% Becase snow plowing is a complex operation, there are many things to keep track of/that should be taken into account. A natural way to divide them would be the constraints that the infrastructure and environment impose on how the work can be done, and what are the requirements set for how the work should be done.
...
% Underlying graph -> graph as set -> required elements.
% the properties each element can/should have
% /each element should have properties. Now to list them ...
